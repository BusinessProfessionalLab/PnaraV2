...
export function MenuBomBuilder() {
  ...
  const catMut = useMutation({
    mutationFn: (name: string) => {
      const displayPriority = Math.max(
        0,
        ...(cats.data ?? [])
...
function ProductForm({ categories }: { categories: Category[] }) {
    ...
    mutationFn: async () => {
      const displayPriority = Math.max(
        ...
        ...(items.data ?? [])
          ...
          .map((item) => item.displayPriority),
      ) + 1;
      const id = await api.createMenuItem({ title, nameEn: nameEn || null, description: null, basePrice: Number(price) * 10, taxInclusive: false, imageUrl: imageUrl || null, displayPriority, categoryId, isActive: true, ticketStation: station, prepTimeMinutes: 4 });
  ...
  return <><AddonSelector addons={addons.data ?? []} selected={selectedAddons} onChange={setSelectedAddons} /><form className="grid gap-2 sm:grid-cols-2" onSubmit={(e) => { e.preventDefault(); mut.mutate(); }}><Input placeholder="نام فارسی محصول" value={title} onChange={(e) => setTitle(e.target.value)} /><Input dir="ltr" placeholder="English name" value={nameEn} onChange={(e) => setNameEn(e.target.value)} /><PriceInput value={price} onChange={setPrice} placeholder="قیمت" /><Select value={categoryId} onValueChange={setCategoryId}><SelectTrigger><SelectValue placeholder="دسته" /></SelectTrigger><SelectContent>{categories.map((c) => <SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>)}</SelectContent></Select><ImageUpload value={imageUrl} onChange={setImageUrl} /><Select value={station} onValueChange={(v) => setStation(v as TicketStation)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="Bar">بار</SelectItem><SelectItem value="Kitchen">آشپزخانه</SelectItem><SelectItem value="KitchenAndBar">هر دو</SelectItem></SelectContent></Select><Button className="sm:col-span-2" type="submit" disabled={!title || !price || !categoryId || mut.isPending}>{mut.isPending ? "در حال ثبت..." : "ثبت محصول"}</Button></form></>;
}
